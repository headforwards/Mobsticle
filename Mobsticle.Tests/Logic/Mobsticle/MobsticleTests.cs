...
namespace Mobsticle.Tests.Logic.Mobsticle
{
    ...
    public class MobsticleTests
    {
        ...
        [TestMethod]
        public void Rotate_MovesToSpecifiedDriver()
        {
            _settings.Participants.Returns(new List<string> { "A", "B", "C", "D", "E" });
            _mobsticle.Settings = _settings;
            _mobsticle.Rotate();
            Assert.IsTrue(_mobsticle.Participants[1].IsDriving == true);
            Assert.IsTrue(_mobsticle.Participants[2].IsDrivingNext == true);
            _mobsticle.Rotate(4);
            Assert.IsTrue(_mobsticle.Participants[4].IsDriving == true);
            Assert.IsTrue(_mobsticle.Participants[2].IsDrivingNext == false);
            Assert.IsTrue(_mobsticle.Participants[0].IsDrivingNext == true);
        }
